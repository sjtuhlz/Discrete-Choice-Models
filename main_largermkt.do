...
/*
*********************
* Use option 4: zone * week (exclude those 7 out of 95 stores in store.dta that do not have an assigned "zone")
* in finaldata.dta, there are 93 stores.
*********************
use mktcp.dta, clear
capture drop _merge
merge m:1 store using store.dta, keepusing(zone)
drop if _merge != 3 // 95 stores are matched, while 39 stores are from mktcp.dta that do not appear in store.dta, no big deal :)
bysort zone: egen mktcp_zone = total(mktcp_store)
save mktcp_zone.dta, replace
*/


capture drop _merge 
merge m:1 store using mktcp_zone.dta, keepusing(zone mktcp_zone)
drop if _merge != 3


** Calculate average price across a zone during a week.
gen pq = price_oz * quant
collapse (sum) sumpq=pq sumq=quant (mean) mktcp_zone, by(prod week zone)
gen avgp =  sumpq / sumq


gen share = sumq / mktcp_zone
bysort week zone: egen sharetot = total(share)
...
keep if _merge == 3
drop _merge
drop tea beer // tea and beer are not present in the top 20 product chosen, so drop them
drop if share == 0  //how come there is 0 share for a product????????????

save finaldata_zone.dta, replace



****************************************
/* ********prepare for matlab input 
** change product string to numeric for Matlab
encode prod, gen(prod_id) //prints in string
gen prod_n = prod_id //prints in number 



** IV, 1-product (early prepartion for matlab)
egen mkt = group(zone week)


** There is one week*zone combinations that has just one product available
* week 34 zone 10.
* All other week*zone combinations (i.e. mkts) have 18,19 or 20 products.
bysort mkt: egen n_mkt = count(mkt)
drop if n_mkt == 1
drop n_mkt
...
levelsof prod_n, local(prod)
capture drop IV*
foreach lc_ctr of local control {
	gen IV_`lc_ctr' = .
	foreach lc_prod of local prod {
		gen a = `lc_ctr'
		replace a = 0 if prod_n == `lc_prod'
		bysort mkt: egen b = total(a)
		replace IV_`lc_ctr' = b if prod_n == `lc_prod'
		drop b a 
	}
}	
...
drop sumpq mktcp_zone sharetot sharedif brand prod_id prod sumq week zone 
outsheet using finaldata_zone_IV.txt, replace 
*******************************************/
...
** IV, 1-brand
levelsof brand, local(brand)

foreach lc_ctr of local control {
	gen IV_`lc_ctr' = .
	gen a = `lc_ctr'
	foreach lc_brd of local brand {
		replace a = 0 if brand == "`lc_brd'"
		bysort mkt: egen b = total(a)
		replace IV_`lc_ctr' = b if brand == "`lc_brd'"
		drop b
	}
	drop a 
}	
...
estimates save berry_iv, replace
...
foreach lc_ctr of local control {
	...
	gen a = `lc_ctr'
	foreach lc_prod of local prod {
		replace a = 0 if prod == "`lc_prod'"
		bysort mkt: egen b = total(a)
		replace IV_`lc_ctr' = b if prod == "`lc_prod'"
		drop b
	}
	drop a 
}	
...
gen IV = .
foreach lc_brd of local brand {
	replace IV = 0 if brand == "`lc_brd'"
	bysort mkt: egen b = mean(price_oz)
	replace IV_`lc_ctr' = b if brand == "`lc_brd'"
	drop b
}
drop a 
...
* IV_free IV_cola IV_other
blp share `control', endog(price=IV_diet) stochastic(diet) markets(mkt) draws(50) tolin(1E-10) tolout(1E-10)
